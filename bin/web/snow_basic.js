...
var Main = function() {
	this.no_gamepad_deadzone = false;
	this.vsync = true;
	this.physical_x = 0;
	this.render_y = 0;
	this.render_x = 0;
...
$hxClasses["Main"] = Main;
Main.__name__ = ["Main"];
Main.__super__ = snow_AppFixedTimestep;
Main.prototype = $extend(snow_AppFixedTimestep.prototype,{
	config: function(config) {
	...
	,ready: function() {
		haxe_Log.trace("     i / main / " + "/ HOST / ready",{ fileName : "Main.hx", lineNumber : 46, className : "Main", methodName : "ready"});
		haxe_Log.trace("     i / main / " + ("OpenGL reports version " + snow_modules_opengl_web_GL.versionString()),{ fileName : "Main.hx", lineNumber : 47, className : "Main", methodName : "ready"});
		haxe_Log.trace("     i / main / " + ("app config is loaded as : " + Std.string(this.app.config.runtime)),{ fileName : "Main.hx", lineNumber : 48, className : "Main", methodName : "ready"});
		haxe_Log.trace("     i / main / " + "init / shaders and buffers ",{ fileName : "Main.hx", lineNumber : 50, className : "Main", methodName : "ready"});
		this.apply_config();
		this.init_shaders();
		this.init_buffers();
		haxe_Log.trace("     i / main / " + "init / done with shaders and buffers",{ fileName : "Main.hx", lineNumber : 56, className : "Main", methodName : "ready"});
		this.render_y = (this.app.window.height - this.size) / 2;
		this.app.window.onrender = $bind(this,this.on_render_update);
	}
	,update: function(delta) {
		this.render_x = this.physical_x;
		this.physical_x += this.speed * this.dir_x * delta;
	}
	,on_render_update: function(window) {
		var prev_render_x = this.render_x;
		this.render_x = this.physical_x * this.alpha + prev_render_x * (1.0 - this.alpha);
		if(this.render_x >= this.app.window.width - this.size) {
			this.render_x = this.app.window.width - this.size;
			this.dir_x = -1;
		} else if(this.render_x <= 0) {
			this.render_x = 0;
			this.dir_x = 1;
		}
		this.draw();
	...
	,draw: function() {
		snow_modules_opengl_web_GL.viewport(0,0,this.app.window.width,this.app.window.height);
		snow_modules_opengl_web_GL.clearColor(1.0,1.0,1.0,1.0);
		snow_modules_opengl_web_GL.clear(16384);
		this.projection = this.createOrthoMatrix(0,this.app.window.width,this.app.window.height,0,1000,-1000);
		this.modelview = this.create2DMatrix(this.render_x,this.render_y,1,0);
		snow_modules_opengl_web_GL.useProgram(this.program);
		snow_modules_opengl_web_GL.uniformMatrix4fv(this.uniform_MP,false,this.projection);
		snow_modules_opengl_web_GL.uniformMatrix4fv(this.uniform_MV,false,this.modelview);
		snow_modules_opengl_web_GL.enableVertexAttribArray(this.attr_pos);
		snow_modules_opengl_web_GL.bindBuffer(34962,this.pos_buffer);
		snow_modules_opengl_web_GL.vertexAttribPointer(this.attr_pos,3,5126,false,0,0);
		snow_modules_opengl_web_GL.drawArrays(5,0,4);
		snow_modules_opengl_web_GL.bindBuffer(34962,null);
		snow_modules_opengl_web_GL.disableVertexAttribArray(this.attr_pos);
		snow_modules_opengl_web_GL.useProgram(null);
	}
	,onkeyup: function(keycode,scancode,_,mod,_1,_2) {
		if(keycode == snow_system_input_Keycodes.enter && mod.alt) this.app.window.set_fullscreen(!this.app.window.get_fullscreen());
		if(keycode == snow_system_input_Keycodes.enter && mod.ctrl) this.app.window.set_grab(!this.app.window.get_grab());
		if(keycode == snow_system_input_Keycodes.escape) this.app.shutdown();
		if(keycode == snow_system_input_Keycodes.key_g) {
			this.no_gamepad_deadzone = !this.no_gamepad_deadzone;
			haxe_Log.trace("     i / main / " + ("gamepad no clamp: " + Std.string(this.no_gamepad_deadzone)),{ fileName : "Main.hx", lineNumber : 159, className : "Main", methodName : "onkeyup"});
		}
		if(keycode == snow_system_input_Keycodes.key_v) {
			this.vsync = !this.vsync;
			this.app.windowing.enable_vsync(this.vsync);
			haxe_Log.trace("     i / main / " + ("vsync enabled : " + Std.string(this.vsync)),{ fileName : "Main.hx", lineNumber : 165, className : "Main", methodName : "onkeyup"});
		}
	}
	,ontouchmove: function(x,y,dx,dy,touch_id,timestamp) {
		this.render_x = this.physical_x = this.app.window.width * x - this.size / 2;
		this.render_y = this.app.window.height * y - this.size / 2;
	}
	,onmousemove: function(x,y,xrel,yrel,timestamp,window_id) {
		if(this.app.input.keydown(snow_system_input_Keycodes.space)) {
			this.render_x = this.physical_x = x - this.size / 2;
			this.render_y = y - this.size / 2;
		}
	}
	,onmouseup: function(x,y,button,timestamp,window_id) {
		this.render_x = this.physical_x = x - this.size / 2;
	...
	,ongamepadaxis: function(gamepad,axis,value,timestamp) {
		if(Math.abs(value) > 0.2 || this.no_gamepad_deadzone) {
			if(axis == 1) this.render_y += value * 2 * this.speed * this.delta_time;
		}
	}
	,apply_config: function() {
		if(this.app.config.runtime.size != null) this.size = this.app.config.runtime.size;
		if(this.app.config.runtime.movespeed != null) this.speed = this.app.config.runtime.movespeed;
		if(this.app.config.runtime.timescale != null) this.timescale = this.app.config.runtime.timescale;
	}
	,init_shaders: function() {
		...
		haxe_Log.trace("     i / main / " + "about to create a vertex shader",{ fileName : "Main.hx", lineNumber : 280, className : "Main", methodName : "init_shaders"});
		var vshader = snow_modules_opengl_web_GL.createShader(35633);
		snow_modules_opengl_web_GL.shaderSource(vshader,vert_shader);
		snow_modules_opengl_web_GL.compileShader(vshader);
		if(snow_modules_opengl_web_GL.getShaderParameter(vshader,35713) == 0) throw new js__$Boot_HaxeError("Error compiling vertex shader" + snow_modules_opengl_web_GL.getShaderInfoLog(vshader));
		haxe_Log.trace("     i / main / " + "vertex shader compiled",{ fileName : "Main.hx", lineNumber : 291, className : "Main", methodName : "init_shaders"});
		haxe_Log.trace("     i / main / " + "about to create a fragment shader",{ fileName : "Main.hx", lineNumber : 294, className : "Main", methodName : "init_shaders"});
		var fshader = snow_modules_opengl_web_GL.createShader(35632);
		snow_modules_opengl_web_GL.shaderSource(fshader,frag_shader);
		snow_modules_opengl_web_GL.compileShader(fshader);
		if(snow_modules_opengl_web_GL.getShaderParameter(fshader,35713) == 0) throw new js__$Boot_HaxeError("Error compiling fragment shader" + snow_modules_opengl_web_GL.getShaderInfoLog(fshader));
		haxe_Log.trace("     i / main / " + "fragment shader compiled",{ fileName : "Main.hx", lineNumber : 305, className : "Main", methodName : "init_shaders"});
		haxe_Log.trace("     i / main / " + "about to link the shader program",{ fileName : "Main.hx", lineNumber : 309, className : "Main", methodName : "init_shaders"});
		this.program = snow_modules_opengl_web_GL.createProgram();
		snow_modules_opengl_web_GL.attachShader(this.program,vshader);
		snow_modules_opengl_web_GL.attachShader(this.program,fshader);
		snow_modules_opengl_web_GL.linkProgram(this.program);
		if(snow_modules_opengl_web_GL.getProgramParameter(this.program,35714) == 0) throw new js__$Boot_HaxeError("Unable to link the shader program: " + snow_modules_opengl_web_GL.getProgramInfoLog(this.program));
		this.attr_pos = snow_modules_opengl_web_GL.getAttribLocation(this.program,"vert_pos");
		this.uniform_MP = snow_modules_opengl_web_GL.getUniformLocation(this.program,"projection");
		this.uniform_MV = snow_modules_opengl_web_GL.getUniformLocation(this.program,"modelview");
	}
	,init_buffers: function() {
		var vertices = [this.size,this.size,0,0,this.size,0,this.size,0,0,0,0,0];
		this.pos_buffer = snow_modules_opengl_web_GL.createBuffer();
		snow_modules_opengl_web_GL.bindBuffer(34962,this.pos_buffer);
	...
	,createOrthoMatrix: function(x0,x1,y0,y1,zNear,zFar) {
		var sx = 1.0 / (x1 - x0);
		var sy = 1.0 / (y1 - y0);
	...
	,create2DMatrix: function(x,y,scale,rotation) {
...
SnowApp.main = function() {
	...
	var _snow_config = { has_loop : true, config_custom_assets : false, config_custom_runtime : false, config_runtime_path : "config.json", config_assets_path : "manifest", app_package : "org.snowkit.snowbasic"};
